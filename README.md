...
If you're on a debian/ubuntu based host with a custom or downstream distro provided kernel (ie. Pop!_OS), the container won't be able to install the kernel headers from the regular ubuntu and debian repos. In those cases, you can try installing the headers on the host via `sudo apt install linux-headers-$(uname -r)` (if distro version) and then add a volume mapping for `/usr/src:/usr/src`, or if custom built, map the location of the existing headers to allow the container to use host installed headers to build the kernel module (tested successful on Pop!_OS, ymmv).   On AMI Linux 2 which does not come with the kernel modules, it is necessary to install headers + sources via `yum install -y kernel-headers-$(uname -r) kernel-devel-$(uname -r)`, and a volume mapping for `/usr/include:/usr/include` in addition to `/usr/src:/usr/src`
